...
@section('content')
    <div class="d-flex align-items-center justify-content-between"> <button type="button" class="btn btn-primary"><a
        ...
        <div class="row g-3 align-items-center">
            <form action="{{ route('room.search') }}" method="post" class="d-flex">
                @csrf
                <div class="col-auto">
        ...
        <tbody>
            @foreach ($rooms as $key => $room)
                <tr>
                    ...
                    <td>{{ $room->name }}</td>
                    <td>{{ $room->row }}</td>
                    <td>{{ $room->seat_number }}</td>
                    <td>{{ $room->created_at }}</td>
                    <td class="whitespace-nowrap">
                        <button type="button" class="btn btn-success"><a href="edit/{{ $room->id }}">Sửa</a></button>
                        <button type="button" class="btn btn-danger"><a onclick=" return confirm('Bạn có chắc chắn xoá?')"
                                href="delete/{{ $room->id }}">Xoá</a></button>
                    </td>
                </tr>
            @endforeach
        </tbody>
